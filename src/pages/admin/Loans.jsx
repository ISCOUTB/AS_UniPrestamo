...
import { getLoans, getResources, getUsers, createLoan, updateLoan } from '../../data/firebaseService';
...
export default function AdminLoans() {
  ...
  useEffect(() => {
    ...
    Promise.all([getLoans(), getResources(), getUsers()]).then(([l, r, u]) => {
      setLoans(l || []);
      setResources(r || []);
      setUsers(u || []);
      setLoading(false);
  ...
  async function handleReturn(id) {
    ...
    try {
      await updateLoan(id, { status: 'returned', returnDate });
      setLoans(prev => prev.map(l => l.id === id ? { ...l, status: 'returned', returnDate } : l));
      showToast('Devolucion registrada exitosamente');
    } catch (error) {
      console.error('Error returning loan:', error);
      showToast('Error al registrar la devolucion', 'error');
    }
  }
  ...
  async function handleCreate() {
    ...
    const typedUser = form.userName.trim().toLowerCase();
    const matchedUser = users.find(u =>
      u.name?.toLowerCase() === typedUser ||
      u.email?.toLowerCase() === typedUser
    );
    const today = new Date().toISOString().split('T')[0];
    const newLoan = {
      resourceId: form.resourceId,
      resourceName: resource?.name || '',
      userId: matchedUser?.uid || matchedUser?.id || form.userName,
      userName: matchedUser?.name || form.userName,
    ...
    try {
      const id = await createLoan(newLoan);
      setLoans(prev => [{ ...newLoan, id }, ...prev]);
      showToast('Prestamo registrado exitosamente');
      setModalOpen(false);
      setForm({ resourceId: '', userName: '', dueDate: '', notes: '' });
    } catch (error) {
      console.error('Error creating loan:', error);
      showToast('Error al registrar el prestamo', 'error');
    }
  }
    ...
    <div>
      ...
      >
        ...
        <Field label="Usuario solicitante *">
          <Input value={form.userName} onChange={e => setForm(f => ({ ...f, userName: e.target.value }))} placeholder="Nombre o correo del usuario" />
        </Field>

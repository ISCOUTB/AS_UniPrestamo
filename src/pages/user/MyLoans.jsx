...
import { getCurrentUser, subscribeToLoans } from '../../data/firebaseService';
...
export default function UserLoans() {
  ...
  useEffect(() => {
    ...
    getCurrentUser().then((u) => {
      if (!mounted) return;
      setCurrentUser(u || {});
      unsubscribeLoans = subscribeToLoans(
        (l) => {
          if (!mounted) return;
          setLoans(l || []);
          setLoading(false);
  ...
  const currentUserName = CURRENT_USER?.name?.trim().toLowerCase();
  const currentUserEmail = CURRENT_USER?.email?.trim().toLowerCase();
  const myLoans = LOANS.filter(l =>
    l.userId === CURRENT_USER?.id ||
    (currentUserName && l.userName?.trim().toLowerCase() === currentUserName) ||
    (currentUserEmail && l.userEmail?.trim().toLowerCase() === currentUserEmail) ||
    (currentUserEmail && l.userName?.trim().toLowerCase() === currentUserEmail)
  );
    ...
    <div>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {activeLoans.map(l => {
              ...
              return (
                <Card key={l.id} className="loan-card" style={{
                  padding: '16px',
      ...
      <Card>
        {returnedLoans.length === 0
          ? <EmptyState icon="🔑" title="Sin historial" subtitle="Aun no tienes prestamos devueltos registrados." />
          : (

...
// ─── BUTTON ──────────────────────────────────────────────────────
export function Btn({ children, variant = 'default', size = 'md', onClick, disabled, type = 'button', style = {}, className = '' }) {
  const base = {
  ...
  return (
    <button className={`app-btn ${className}`.trim()} type={type} onClick={onClick} disabled={disabled} style={{ ...base, ...variants[variant] }}>
      {children}
    </button>
  );
}
...
export function Card({ children, style = {}, className = '' }) {
  return (
    <div className={`app-card ${className}`.trim()} style={{
  ...
  return (
    <div
      ...
      <div className="app-modal-panel" style={{
        ...
        <div className="app-modal-header" style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-secondary)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div className="app-modal-body" style={{ padding: '20px 24px' }}>{children}</div>
        {footer && <div className="app-modal-footer" style={{ padding: '16px 24px', borderTop: '1px solid var(--border-secondary)', display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>{footer}</div>}
      </div>
    </div>
  );
}
...
export function SearchBar({ value, onChange, placeholder = 'Buscar...' }) {
  return (
    <div className="app-search" style={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }}>
